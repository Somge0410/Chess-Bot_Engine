...
bool Engine::save_tt(const std::string& filename) const {
    std::ofstream file(filename, std::ios::binary);
    if (!file.is_open()) return false;

    // Write the number of entries
    size_t size = this->transposition_table.size();
    file.write(reinterpret_cast<const char*>(&size), sizeof(size));

    // Loop and write each member of each entry MANUALLY
    for (const auto& pair : this->transposition_table) {
        const uint64_t& hash = pair.first;
        const TTEntry& entry = pair.second;
        const Move& move = entry.best_move;

        // Write the hash
        file.write(reinterpret_cast<const char*>(&hash), sizeof(hash));
        
        // Write all the plain data from the TTEntry
        ...
        file.write(reinterpret_cast<const char*>(&entry.flag), sizeof(entry.flag));

        // --- Manually Serialize the Move object ---
        file.write(reinterpret_cast<const char*>(&move.from_square), sizeof(move.from_square));
        file.write(reinterpret_cast<const char*>(&move.to_square), sizeof(move.to_square));
        // ... write all other plain data members of Move ...

        // --- Special Handling for std::string ---
        size_t str_size = move.old_castling_rights.length();
        file.write(reinterpret_cast<const char*>(&str_size), sizeof(str_size)); // Write length
        file.write(move.old_castling_rights.c_str(), str_size);                // Write data
    }
    return true;
}

bool Engine::load_tt(const std::string& filename) {
    ...
    // Loop and read each member of each entry MANUALLY
    for (size_t i = 0; i < size; ++i) {
        ...
        // Read all the plain data for the TTEntry
        ...
        file.read(reinterpret_cast<char*>(&entry.flag), sizeof(entry.flag));

        // --- Manually Deserialize the Move object ---
        file.read(reinterpret_cast<char*>(&move.from_square), sizeof(move.from_square));
        file.read(reinterpret_cast<char*>(&move.to_square), sizeof(move.to_square));
        // ... read all other plain data members of Move ...
        
        // --- Special Handling for std::string ---
        size_t str_size;
        file.read(reinterpret_cast<char*>(&str_size), sizeof(str_size)); // Read length
        std::string rights(str_size, ' ');
        file.read(&rights[0], str_size);                              // Read data
        move.old_castling_rights = rights;

        entry.best_move = move;
        this->transposition_table[hash] = entry;
    }
    return true;
}