...
private:
    ...
    static void generate_knight_moves(MoveList& moves, const Board& board, Color own_color,const uint64_t& pinned_info,uint64_t remedy_mask=BOARD_ALL_SET, bool captures_only=false,bool with_checks=false);
    static void generate_pawn_moves(MoveList& moves, const Board& board, Color own_color,const int king_square,const uint64_t& pinned_info,const uint64_t& remedy_mask=BOARD_ALL_SET, bool captures_only=false,bool with_checks=false);
    static void generate_sliding_moves(MoveList& moves, PieceType piece,const Board& board, Color own_color, const uint64_t& pinned_info, const uint64_t& remedy_mask=BOARD_ALL_SET, bool captures_only=false);
    static void generate_pawn_pushes(MoveList& moves,const Board& board,Color own_color,const uint64_t& pinned_info,uint64_t remedy_mask=BOARD_ALL_SET,bool with_checks=false);
